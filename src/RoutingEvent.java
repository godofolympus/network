...
//	}
	...
	public List<Event> handle() {
		...
		ChangeRoutingEvent changeRoutingEvent = new ChangeRoutingEvent(this.time + Constants.ROUTING_TIMEOUT, routers);
	...
	private void initializeBellmanFord(Router r) {
		...
		for (Link link : r.links.values()) {
			...
			r.distances.put(adjComponent.name, link.totalLeftDelay + link.totalRightDelay);
