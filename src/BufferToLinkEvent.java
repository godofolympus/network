...
public class BufferToLinkEvent extends Event {
	...
	public List<Event> handle() {
		...
		if (direction == Constants.Direction.RIGHT) {
			...
			if ((nextPacket = link.leftBuffer.peek()) != null) {
				newEvents.add(new BufferToLinkEvent(
						...
						nextPacket, Constants.Direction.RIGHT));
			}
		} else {
			...
			if ((nextPacket = link.rightBuffer.peek()) != null) {
				newEvents.add(new BufferToLinkEvent(
						this.time + nextPacket.size / link.linkRate, link,
						nextPacket, Constants.Direction.LEFT));
			}
		}
		 ...
		 * Constants.Direction nextDir; Packet nextPacket; double delay;
		 * 
		 * if (link.leftBuffer.isEmpty()) { // Left buffer is empty, choose the
		 * packet on the right nextDir = Constants.Direction.LEFT; nextPacket =
		 * link.rightBuffer.peek();
		 * 
		 * } else if (link.rightBuffer.isEmpty()) { // Right buffer is empty,
		 * choose the packet on the left nextDir = Constants.Direction.RIGHT;
		 * nextPacket = link.leftBuffer.peek();
		 * 
		 * } else { // Both buffers have elements, choose the packet that
		 * arrived // first double leftTime = link.leftArrivalTimes.peek();
		 * double rightTime = link.rightArrivalTimes.peek();
		 * 
		 * if (leftTime < rightTime) { nextDir = Constants.Direction.RIGHT;
		 * nextPacket = link.leftBuffer.peek(); } else { nextDir =
		 * Constants.Direction.LEFT; nextPacket = link.rightBuffer.peek(); } }
		 * 
		 * // If the next packet is going the same direction, then we only //
		 * consider propagation delay. If we are changing directions, then // we
		 * also need to consider link delay. if (nextDir == direction) { delay =
		 * nextPacket.size / link.linkRate; } else { delay = link.linkDelay +
		 * nextPacket.size / link.linkRate; }
		 * 
